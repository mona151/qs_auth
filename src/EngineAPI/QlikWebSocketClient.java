...
public class QlikWebSocketClient extends WebSocketClient {
    ...
    public QlikWebSocketClient(String wssServerUrl, AuthCertificate qlikCert) throws Exception {
        super(new URI(wssServerUrl),
                Map.of("X-Qlik-User", "UserDirectory=QLIKSENSEI; UserId=testnovartis"));
    ...
    public JSONObject connectBlockingCommand() throws ExecutionException, InterruptedException {
        ...
        return new JSONObject(responseFuture.get());
    }

    public JSONObject sendCommand(String command) throws ExecutionException, InterruptedException {
        // CompletableFutures can only be "used" once, so create a new object
        responseFuture = new CompletableFuture<>();
        this.send(command);
        // Keep in mind potential errors and a timeout value to this call
